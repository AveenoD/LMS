...
import { registerTenantSchema, suspendSchema } from '../validators/superadmin.validators.js';
...
router.patch('/tenants/:id/suspend', validate(suspendSchema), ctrl.suspendTenant);

// Subscriptions
router.get('/subscriptions', ctrl.listSubscriptions);
router.get('/subscriptions/expiring', ctrl.expiring);
...
router.patch('/leads/:id/read', leadCtrl.markRead);
router.patch('/leads/:id/status', validate(updateLeadStatusSchema), leadCtrl.updateStatus);
