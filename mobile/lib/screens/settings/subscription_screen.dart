...
class _SubscriptionScreenState extends ConsumerState<SubscriptionScreen> {
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    ...
    try {
      ...
      await verifyPayment(api, _orderId!, response.paymentId ?? 'mock', response.signature);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment Successful! Subscription renewed.')));
  ...
  Future<void> _startPaymentFlow() async {
    ...
    try {
      ...
      setState(() => _isProcessing = false);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error initiating payment: $e')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Subscription & Billing', style: TextStyle(color: Colors.white)),
        backgroundColor: const Color(0xFF1F2E27),
        iconTheme: const IconThemeData(color: Colors.white),
      ),
      body: subAsync.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (err, stack) => Center(child: Text('Error loading subscription: $err')),
        data: (sub) {
          ...
          if (isTrial) {
            ...
            ended = daysLeft < 0 || status == 'trial_expired' || status == 'past_due' || status == 'expired';
          } else {
            ...
            ended = daysLeft < 0 || status == 'past_due' || status == 'expired';
          }

          final statusColor = ended ? Colors.red : (isTrial ? Colors.orange : Colors.green);
          final statusText = ended ? 'Expired' : (isTrial ? 'Trial Active ($daysLeft days left)' : 'Active ($daysLeft days left)');
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Text(planName, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF1F2E27))),
                            Container(
                        ...
                        _buildInfoRow('Billing Cycle', billingCycle.toString().toUpperCase()),
                        const Divider(height: 32),
                        if (sub['perStudentRate'] != null)
                          _buildInfoRow('Rate Per Student', '₹${sub['perStudentRate']} / cycle'),
                        if (sub['perStudentRate'] == null)
                          _buildInfoRow('Flat Plan Amount', '₹${sub['amount']}'),
                        const Divider(height: 32),
                        if (isTrial && trialEndsAt != null)
                          _buildInfoRow('Trial Ends On', DateFormat('MMM dd, yyyy').format(trialEndsAt)),
                        if (!isTrial && nextBillingDate != null)
                          _buildInfoRow('Next Billing Date', DateFormat('MMM dd, yyyy').format(nextBillingDate)),
                      ],
                    ),
                  ),
                ),
  ...
  Widget _buildInfoRow(String label, String value) {
    return Row(
      ...
      children: [
        ...
        Text(value, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 16, color: Color(0xFF1F2E27))),
      ],
    );
  }
}
