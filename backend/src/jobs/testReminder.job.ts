...
/**
 * Runs every SWEEP_WINDOW_MINUTES: finds tests whose scheduled_at is between
...
/** Schedule every 10 minutes — fine-grained enough that a 1-hour-before
 *  reminder lands within a ~10 minute window of the target time. */
export function scheduleTestReminderJob(): void {
  cron.schedule(`*/${SWEEP_WINDOW_MINUTES} * * * *`, () => {
    runTestReminderSweep().catch((err) =>
      logger.error('Test reminder sweep failed', { error: err instanceof Error ? err.message : String(err) })
    );
  });
  logger.info(`Test reminder cron scheduled (every ${SWEEP_WINDOW_MINUTES} min)`);
}
